...
This study shows the accuracy of FEM and also its advantages in handling complex geometries.
\fi
...
%This model couples the diffusion of free chloride ions with an adsorption process. 
...
\Rd{Briefly introduce initial conditions... typically 0 for all but not necessarily}
...
Particularly, we use the Lie splitting \cite{} \Rd{Ramin add this} in which for a given time step, the reaction-diffusion system is solved first while neglecting the effects of adsorption, after which the adsorption equation is solved independently using the just-computed results from the reaction-diffusion system.
